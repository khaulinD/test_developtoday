...
async def update_target(target_id: int, status: bool | None = None):
    await TargetsController.update_target_status(
        target_id=target_id, status=status
    )
    return JSONResponse(
        status_code=200, content={"status": "Status update successful"}
    )
...
async def update_target_note(target_id: int, note: TargetsNoteSchema):
    ...
    res = await TargetsController.update_target_note(
        target_id=target_id, new_note=note.note
    )
    return res
