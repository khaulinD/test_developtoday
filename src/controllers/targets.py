...
class TargetsController:
    @staticmethod
    @db_session
    async def update_target_status(session, target_id: int, status: bool = False):
        res = await session.execute(update(Target).where(Target.id == target_id).values(complete_state=status))
    ...
    async def update_target_note(session, target_id: int, new_note: str):
        ...
        if target.complete_state or (target.mission and target.mission.complete_state):
            raise HTTPException(
